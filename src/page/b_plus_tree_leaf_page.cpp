 ...
 * 二分查找，返回第一个大于等于key的index，如果所有key都小于输入key，返回size
 */
...
int LeafPage::Insert(GenericKey *key, const RowId &value, const KeyManager &KM) {
  ...
  //中间插入，需要右移
  if(idx<GetSize()){
    int move_num=GetSize()-idx;
    PairCopy(pairs_off+(idx+1)*pair_size, pairs_off+idx*pair_size, move_num);
  }
  SetKeyAt(idx, key);
...
void LeafPage::MoveHalfTo(LeafPage *recipient) {
  ...
  recipient->CopyNFrom(pairs_off + split_index * pair_size, move_num);
  SetSize(split_index);
}
...
void LeafPage::CopyNFrom(void *src, int size) {
  void* dest = pairs_off + GetSize() * pair_size;
...
bool LeafPage::Lookup(const GenericKey *key, RowId &value, const KeyManager &KM) {
  ...
  if(KM.CompareKeys(key, KeyAt(idx))==0){
    value=ValueAt(idx);
    return true;
  }
  return false;
...
int LeafPage::RemoveAndDeleteRecord(const GenericKey *key, const KeyManager &KM) {
  ...
  if (idx < GetSize() - 1) {
    PairCopy(pairs_off + idx * pair_size, pairs_off + (idx + 1) * pair_size, GetSize() - idx - 1);
  }
  IncreaseSize(-1);
  return GetSize();
}
...
void LeafPage::MoveFirstToEndOf(LeafPage *recipient) {
  ...
  if (GetSize() > 1) {
    PairCopy(pairs_off, pairs_off + pair_size, GetSize() - 1);
  }
  IncreaseSize(-1);
}
