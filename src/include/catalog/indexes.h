...
class IndexMetadata {
 ...
 private:
  static constexpr uint32_t INDEX_METADATA_MAGIC_NUM = 344528;//魔法数
  index_id_t index_id_;//全局id
  std::string index_name_;
  table_id_t table_id_;//依附的表id
  std::vector<uint32_t> key_map_; /** The mapping of index key to tuple key *///依附的列
};
 ...
 */
  void Init(IndexMetadata *meta_data, TableInfo *table_info, BufferPoolManager *buffer_pool_manager) {
    ...
    std::vector<uint32_t> key_map = meta_data->GetKeyMapping();
    //浅拷贝省内存
 ...
 private:
  IndexMetadata *meta_data_;//元数据
  Index *index_;//内存中建的索引bptree
  IndexSchema *key_schema_;//索引列的schema
};
