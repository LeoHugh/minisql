...
class BPlusTree {
 ...
 private:
  ...
  // member variable
  index_id_t index_id_;  //索引唯一标识
  page_id_t root_page_id_{INVALID_PAGE_ID};  //根节点的物理页号
  BufferPoolManager *buffer_pool_manager_;   // buffer pool manager的指针
  KeyManager processor_;  //管理key的序列化和比较
  int leaf_max_size_;     //叶子节点的最大容量，单位是key-value对的数量，初始化时传入，默认值为UNDEFINED_SIZE，表示需要根据page size和key size计算得到
  int internal_max_size_; //内部节点的最大容量，单位是key-value对的数量，初始化时传入，默认值为UNDEFINED_SIZE，表示需要根据page size和key size计算得到
};
