...
class TableHeap {
 ...
 private:
  ...
  explicit TableHeap(BufferPoolManager *buffer_pool_manager, Schema *schema, Txn *txn, LogManager *log_manager,
    ...
    auto page_raw = buffer_pool_manager->NewPage(first_page_id_);
    if (page_raw == nullptr) {
      throw std::runtime_error("Failed to allocate first page for table heap.");
    }
    auto page = reinterpret_cast<TablePage *>(page_raw);
    page->Init(first_page_id_, PAGE_SIZE, log_manager, txn);
    buffer_pool_manager->UnpinPage(first_page_id_, true);
    AddPageToFreeList(first_page_id_);
  };
 ...
 private:
  void AddPageToFreeList(page_id_t page_id) {
    if (page_id == INVALID_PAGE_ID) return;
    if (std::find(free_page_ids_.begin(), free_page_ids_.end(), page_id) == free_page_ids_.end()) {
      free_page_ids_.push_back(page_id);
    }
  }

  void RemovePageFromFreeList(page_id_t page_id) {
    if (page_id == INVALID_PAGE_ID) return;
    auto it = std::remove(free_page_ids_.begin(), free_page_ids_.end(), page_id);
    free_page_ids_.erase(it, free_page_ids_.end());
  }

 private:
  BufferPoolManager *buffer_pool_manager_; //内存管理器
  page_id_t first_page_id_;  //表的第一个页面id
  Schema *schema_;
  [[maybe_unused]] LogManager *log_manager_;
  [[maybe_unused]] LockManager *lock_manager_;
  std::deque<page_id_t> free_page_ids_;  //空闲page，一个优化设计
};
